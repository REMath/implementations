...
Definition toyfun f P Q :=
  Forall iret, safe @ (EIP~=iret ** EDX? ** Q)
          -->> safe @ (EIP~=f ** EDX~=iret ** P).
...
Lemma toyfun_call f P Q:
  |> toyfun f P Q |-- basic P (call_toyfun f) Q @ EDX?.
Proof.
  ...
  - eapply basic_basic; first apply MOV_RI_rule.
    - by ssimpl.
    - done.
  rewrite /basic. rewrite /memIs /=. specintros => i _ _ <- ->.
...
Proof.
  ...
  eapply basic_seq.
  - eapply basic_basic; first apply INC_R_rule.
    - rewrite /OSZCP. by ssimpl.
    done.
  eapply basic_basic; first apply INC_R_rule.
  - rewrite /OSZCP. by ssimpl.
  rewrite addIsIterInc /OSZCP. by sbazooka.
Qed.
...
Example toyfun_example_caller_correct a f:
  Forall a', toyfun f (EAX ~= a') (EAX ~= a' +# 2)
  |-- basic (EAX ~= a) (toyfun_example_caller f) (EAX ~= a +# 4) @ EDX?.
Proof.
...
Example toyfun_example_correct entry (i j: DWORD) a:
  |-- (
    ...
    ) @ (EDX? ** OSZCP_Any) <@ (i--j :-> toyfun_example entry).
Proof.
  rewrite /toyfun_example. rewrite /memIs /programMemIs -/programMemIs.
  ...
  (* The following rewrite underneath a @ is essentially a second-order frame
     rule application. *)
  setoid_rewrite toyfun_example_caller_correct.

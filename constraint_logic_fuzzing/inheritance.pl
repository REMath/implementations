:- module('inheritance', [basicGeneration/2, emptyState/2]).
...
astContains(A, PlacementClause, exp, stmt) :-
    call(PlacementClause, A). % expressions are a kind of statement
...
% 2.) Optionally set its prototype
% 3.) Optionally add to its prototype
% 4.) Create a new instance and bind it to a variable

% introduces a function to be used in an inheritance hierarchy
inherFunction(Name, functionDecl(Name, _, _)).

% introduces an addition to a function in the inheritance hierarchy
inherPrototypeAdd(Name, simpleUpdate(access(var(Name), str('prototype')), str(_), _)).
        
inherPrototypeSet(Name, simpleUpdate(var(Name), str('prototype'), Set)) :-
        Set = object(_); Set = new(_, _).

inherNew(Name, new(var(Name), _)).

optional(Clause) :-
        call(Clause).
optional(_).

optional1(Clause, Param) :-
        call(Clause, Param).
optional1(_, _).

templateInhabited(State, Template) :-
	nb_setval(inst, false),
	format('TRYING: ~w~n', [Template]),
	catch(call_with_time_limit(1, once(statement(Template, State, OutputState))), time_limit_exceeded, false),
	bound(OutputState, Bound),
	(Bound > 0 -> format('BOUND: ~w~n', [Bound]); true),
	nb_setval(inst, true), false.
templateInhabited(_, _) :-
	nb_getval(inst, true).
...
inherMinimal(S) :-
    inSequence([inherFunction(Name),
		inherPrototypeAdd(Name),
		=(Rest)], S),
    astContains(Rest, inherNew(Name), exp, stmt).

makeTemplates :-
    emptyState(State, 9),
    call_with_depth_limit(inherMinimal(Template), 5, Fail),
    Fail \== depth_limit_exceeded,
    templateInhabited(State, Template),
    format('~w~n', [Template]).

basicGeneration(State, Statement) :-
	call_with_depth_limit(inherMinimal(Statement), 7, Fail),
	Fail \== depth_limit_exceeded,
...
calcGenerationRate :-
	...
	call_with_depth_limit(inherMinimal(Template), 5, Fail),
	Fail \== depth_limit_exceeded,
	statement(Template, State, _),
	write(1), nl, fail.

generator(InitialState, S) :-
        call_with_depth_limit(inher(S), 7, Fail),
        Fail \== depth_limit_exceeded,
...
go(Bound) :-
        ...
        writeStatements(generator(IState), 'inheritance', '.js').

...
:- use_module('eval_binop2_clp.pl', [overflowStatement/1 as clpStatement]).
...
prePostIncDec(preIncVar(_)) :- maybe(0.6273772394737143).
prePostIncDec(preIncUpd(_, _)) :- maybe(0.7160344124471081).
prePostIncDec(postIncVar(_)) :- maybe(0.8).
prePostIncDec(postIncUpd(_, _)) :- maybe(0.2).
prePostIncDec(preDecVar(_)) :- maybe(0.6889217401164002).
prePostIncDec(preDecUpd(_, _)) :- maybe(0.5142761691150193).
prePostIncDec(postDecVar(_)) :- maybe(0.3452148739992035).
prePostIncDec(postDecUpd(_, _)) :- maybe(0.23246473765667497).
...
validCatchFinally(none, some(_)) :- maybe(0.4719331401619181).
validCatchFinally(some(_), none) :- maybe(0.5384235778012604).
validCatchFinally(some(_), some(_)) :- maybe(0.5351816394663469).

unop(void) :- maybe(0.2).
unop(typeof) :- maybe(0.2).
unop(uplus) :- maybe(0.8).
unop(uminus) :- maybe(0.7390941852414808).
unop(bitnot) :- maybe(0.8).
unop(lognot) :- maybe(0.4547189555248858).

compoundBinop(plus) :- maybe(0.8).
compoundBinop(minus) :- maybe(0.8).
compoundBinop(mul) :- maybe(0.2).
compoundBinop(div) :- maybe(0.8).
compoundBinop(lshift) :- maybe(0.7999862771129209).
compoundBinop(rshift) :- maybe(0.8).
compoundBinop(urshift) :- maybe(0.2).
compoundBinop(bitand) :- maybe(0.7861810655900247).
compoundBinop(bitor) :- maybe(0.3510977617452475).
compoundBinop(bitxor) :- maybe(0.8).

binop(A) :-
	maybe(0.8),
        compoundBinop(A).
binop(lt) :- maybe(0.8).
binop(le) :- maybe(0.6978251538466589).
binop(gt) :- maybe(0.2).
binop(ge) :- maybe(0.8).
binop(eq) :- maybe(0.6215169938755082).
binop(ne) :- maybe(0.34292782608938877).
binop(equiv) :- maybe(0.5523131833792094).
binop(nequiv) :- maybe(0.7702538285955212).
binop(logand) :- maybe(0.8).
binop(logor) :- maybe(0.35261572964658094).
binop(in) :- maybe(0.5586161819623634).
binop(comma) :- maybe(0.6982154747919281).
binop(instanceof) :- maybe(0.2).
...
skewedRandom(500) :- maybe(0.8).
skewedRandom(-500) :- maybe(0.2).
skewedRandom(N) :- maybe(0.2), random(-500, 500, N).

expression(num(N), State, State) :- maybe(0.8), skewedRandom(N).
expression(bool(_), State, State) :- maybe(0.2).
expression(str(_), State, State) :- maybe(0.06827807252856044).
expression(null, State, State) :- maybe(0.2).
...
genAsts(State, NumAsts, Seen, Result) :-
	...
	(clpStatement(Statement) -> Add = 1; Add = 0); % duplicate checking is too hard
	Add = 0), !,
    NewSeen is Seen + Add,
    NewNum is NumAsts - 1,
...
doEvaluation(Secs) :-
        emptyState(State, 6),
        doesEvaluation(statement_(State), clpStatement, makeUnique, Secs, result(TotalSto, UniqueSto, InterestingSto, TotalCLP)),

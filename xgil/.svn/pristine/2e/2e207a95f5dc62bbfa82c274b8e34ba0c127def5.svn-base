...
// ACTION(..) is code to execute for action trace diagnostics, i.e. "-tr action"
  ...
  #define TRSACTION(stuff) if (trAction) { cout << stuff << endl << flush; }
#else
  #define ACTION(stmt)
...
{
#ifndef NDEBUG
  bool trAction = tracingSys("action");
#endif
...
GLR::GLR(UserActions *user, ParseTables *t)
  : userAct(user),
    ...
    pathQueue(t),
    errorCount(0),
    baseErrorTokenNumber(5),
    errorTokenNumber(0),
    noisyFailedParse(true),
    trAction(tracingSys("action")),
...
void GLR::glrParse(LexerInterface &lexer, SemanticValue &treeTop)
{
  #ifdef NDEBUG
  trace("action") << "warning: NDEBUG must not be defined to use 'action' tracing\n";
  trace("parse")  << "warning: NDEBUG must not be defined to use 'parse' tracing\n";
  #endif
  ...
  //CycleTimer timer;
  innerGlrParse(*this, lexer, treeTop);
  ...
  ::innerGlrParse(GLR &glr, LexerInterface &lexer, SemanticValue &treeTop)
{
  ...
  tryDeterministic:
    ...
    if (topmostParsers.length() == 1) {
      ...
      if (tables->isReduceAction(action)) {
        ...
        afterGeneralLoop:
          ...
          // see if the user wants to keep this reduction. if not we have
          // an error and try error recovery. if recovery succeeds we are
          // left with a new single stack and can fall through and try to
          // keep shifting.
          if (!userAct->keepNontermValue(prodInfo.lhsIndex, sval)) {
            TRSACTION("  CANCEL " <<
                      symbolDescription(newNode->getSymbolC(), userAct, sval));

            // keep a reference on the new node.
            RCPtr<StackNode> singleNode(newNode);

            // clear topmost parsers and remove its reference on the new node.
            topmostParsers.pop();
            newNode->decRefCt();
            xassertdb(newNode->referenceCount == 1);

            glr.recoverParse(singleNode);
          }
          else {
            // emit some trace output
            TRSACTION("  SUCCESS " <<
    ...
    glr.nondeterministicParseToken();
  ...
  getNextToken:
    ...
    // get the next token
    nextToken(&lexer);
    if (glr.errorTokenNumber)
      glr.errorTokenNumber--;
...
// far as I can tell!)
void GLR::nondeterministicParseToken()
{
  ...
  // hang onto a single node from the parsers in case we need
  // to do error recovery.
  RCPtr<StackNode> singleNode(topmostParsers[0]);
  ...
  if (topmostParsers.isEmpty())
    recoverParse(singleNode);
}

// try to recover a parse which failed in the specified state with the
// specified end of stack. the topmost parsers must be empty at start,
// and if recovery is successful a single parser will be added which
// should be able to be reduced.
void GLR::recoverParse(StackNode *singleNode)
{
  xassert(topmostParsers.isEmpty());

  // only print the error message if we are fully recovered.
  if (errorTokenNumber == 0) {
    errorCount++;
    printParseErrorMessage(singleNode->state);
  }

  // look for the nearest state on the stack with error recovery.
  // don't do an exhaustive exploration in case of ambiguity,
  // just follow one path through the stack.

  StackNode *parNode = singleNode;
  while (parNode) {
    StateId state = parNode->state;

    ErrorEntry entry = tables->errorEntry(state, lexerPtr->type);
    if (entry != missingErrorEntry)
      break;

    TRSACTION("  ERROR_POP " <<
              symbolDescription(parNode->getSymbolC(), userAct, parNode->firstSib.sval));

    parNode = parNode->firstSib.sib;
  }

  xassert(parNode);

  TRSACTION("  ERROR_LEFT " <<
            symbolDescription(parNode->getSymbolC(), userAct, parNode->firstSib.sval));

  LexerInterface::NextTokenFunc nextToken = lexerPtr->getTokenFunc();

  bool shiftError = false;

  while (lexerPtr->type != 0) {
    ErrorEntry entry = tables->errorEntry(parNode->state, lexerPtr->type);
    xassert(entry != missingErrorEntry);

    if (entry != advanceErrorEntry) {
      xassert(entry == shiftErrorEntry);
      shiftError = true;
      break;
    }
  ...
  if (!shiftError) {
    // we shifted the remainder of the rest of the input and still didn't
    // find a place to recover from. just scan for some entry in the
    // state table which can be recovered from.
    for (int type = 1; type < tables->getNumTerms(); type++) {
      ErrorEntry entry = tables->errorEntry(parNode->state, type);
      xassert(entry != missingErrorEntry);
  ...
  xassert(lexerPtr->type);

  // construct the new state of the stack. this is parNode with
  // an error terminal shifted on. the lookahead is the token we
  // were scanning for, which will appear right after the 'error'
  // in a production for parNode.

  addTopmostParser(parNode);

  // shift the error and following token onto the top state.
  rwlShiftTerminals(true);
  rwlShiftTerminals();

  // set the error counter so that we know we are in recovery.
  errorTokenNumber = baseErrorTokenNumber;
}
...
void GLR::cleanupAfterParse(SemanticValue &treeTop)
{
...
// shift the current token, if they can. if is_error is set then
// an error token is shifted instead of the next lexer token.
void GLR::rwlShiftTerminals(bool is_error)
{
  int term = is_error ? tables->errorTerm : lexerPtr->type;
  ...
  while (prevTopmost.isNotEmpty()) {                
    ...
    ActionEntry action =
      tables->getActionEntry(leftSibling->state, term);
    ...
    if (tables->isShiftAction(action)) {
      // unambiguous shift
      newState = tables->decodeShift(action, term);
    }
    else if (tables->isReduceAction(action) ||
    ...
    SemanticValue sval = is_error ? 0 : lexerPtr->sval;
    if (prev) {
      // the 'sval' we just grabbed has already been claimed by
      // 'prev->sval'; get a fresh one by duplicating the latter
      sval = userAct->duplicateTerminalValue(term, prev->sval);
      
      TRSACTION("  " << userAct->terminalDescription(term, sval) <<
                " is (@lexer) DUP of " <<
                userAct->terminalDescription(term, prev->sval));
    }

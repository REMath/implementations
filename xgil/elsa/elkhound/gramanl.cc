...
void ItemSet::getPossibleErrorReductions(ProductionList &reductions,
                                         Terminal *errorTerm) const
{
  for (int ind = 0; ind < kernelItems.count(); ind++) {
    const DottedProduction *dprod = kernelItems.nthC(ind)->dprod;

    if (dprod->afterDot && dprod->afterDot->ifTerminal() == errorTerm)
      reductions.append(const_cast<Production*>(dprod->getProd()));
  }

  for (int ind = 0; ind < nonkernelItems.count(); ind++) {
    const DottedProduction *dprod = nonkernelItems.nthC(ind)->dprod;
...
void GrammarAnalysis::computeDProdFirsts()
{
  ...
  FOREACH_PRODUCTION(productions, prodIter) {
    ...
    for (int posn=0; posn <= rhsLen; posn++) {
      const Production *prod = prodIter.data();
      DottedProduction *dprod = getDProd_nc(prod, posn);

      // compute its first
      RHSEltListIter symIter(prod->right, posn);
...
void GrammarAnalysis::computeParseTables(bool allowAmbig)
{
  tables = new ParseTables(numTerms, numNonterms, itemSets.count(), numProds,
                           errorTerm->termIndex, startState->id,
  ...
  FOREACH_OBJLIST(ItemSet, itemSets, stateIter) {
    ...
    for (int termId=0; termId < numTerms; termId++) {
      ...
      if (actions >= 2) {
        ...
        // fill in the actions
        if (shiftDest) {
          set.push(tables->encodeShift(shiftDest->id));
        }
        SFOREACH_PRODUCTION(reductions, prodIter) {
          set.push(tables->encodeReduce(prodIter.data()->prodIndex));
        }
        xassert(set.length() == actions);

        cellAction = tables->encodeAmbig(set);
      }
    ...
    // add any error reductions.
    ProductionList errorReductions;
    state->getPossibleErrorReductions(errorReductions, errorTerm);

    if (errorReductions.count() != 0) {
      for (int ind = 0; ind < errorReductions.count(); ind++) {
        Production *error_prod = errorReductions.nth(ind);
        xassert(error_prod->errorRecovery != NULL);

        // fill in reduce error entry.
        int termId = error_prod->errorRecovery->termIndex;
        tables->setErrorEntry(state->id, termId, shiftErrorEntry);
      }

      // fill in remainder of terminals with advance error entry.
      for (int termId=0; termId < numTerms; termId++) {
        ErrorEntry entry = tables->errorEntry(state->id, termId);
        if (entry == missingErrorEntry)
          tables->setErrorEntry(state->id, termId, advanceErrorEntry);
      }
    }

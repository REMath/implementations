...
Section LogicLemmas.
  ...
  Proof.
    ...
    - by ssimpl.
    rewrite /regAny /OSZCP /flagAny. by sbazooka.
  Qed.
End LogicLemmas.

Section LogicRules.
  Theorem triple_assign Q e x S:
    S |-- triple Q[e//x] (Cassign x e) Q.
  Proof.
  ...
  Proof.
    ...
    rewrite lentails_eq. rewrite <-HPP'. by rewrite -lentails_eq.
  Qed.
  ...
  Proof.
    ...
    eapply basic_seq; rewrite -/compile_cmd -/programMemIs.
    - apply H1.
    - apply H2.
  Qed.

  Theorem triple_while S P e C:
    S |-- triple ((fun s => (eeval e s == zero _) = false) //\\ P) C P ->
    S |-- triple P (Cwhile e C) ((fun s => (eeval e s == zero _) = true) //\\ P).
  Proof.
    rewrite /triple. autorewrite with push_at. move=> HC. simpl compile_cmd.
    set I := fun b:bool =>
      asn_denot ((fun s => (eeval e s == zero _) = b) //\\ P) **
  ...
  Theorem triple_if S P e C1 C2 Q:
    S |-- triple ((fun s => (eeval e s == zero _) = false) //\\ P) C1 Q ->
    S |-- triple ((fun s => (eeval e s == zero _) = true) //\\ P) C2 Q ->
  ...
  Lemma subst_true_special_case e (x: var): |-- ltrue[e // x].

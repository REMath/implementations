...
void astParseTerminals(Environment &env, TF_terminals const &terms)
{
  // basic declarations
  {
    ...
    // add the error terminal.
    env.g.declareToken(env.g.errorName, maxCode + 1, env.g.errorName);
    env.g.errorTerm = env.g.findTerminal(env.g.errorName);
    env.g.errorTerm->type = "void";
  }
                        ...
                        ProdDecl const *prodDecl)
{
  ...
  if (prodDecl->kind == PDK_ERROR) {
    // add the 'error' terminal at the start of the rhs.
    xassert(env.g.errorTerm);
    prod->append(env.g.errorTerm, env.g.errorName);
  }
  ...
  if (prodDecl->kind == PDK_ERROR) {
    // handling for error productions.

    xassert(prod->right.count() == 2);
    Symbol *sym = prod->right.nthC(1)->sym;

    prod->errorRecovery = sym->ifTerminal();
    if (!prod->errorRecovery)
      astParseError(sym->name, "symbol used in error production must be terminal");
  }
...
void mergeProduction(TF_nonterm *base, ProdDecl *ext)
{
  ...
  case PDK_ERROR:
    if (ext->rhs.count() != 1) {
      astParseError(ext->loc,
                    "'error' production must include exactly one RHS element");
    }
    // fall through.

...
(*---------------------------------------------------------------------------
    Multiplication
    For now, we go via nat. Do not use on large numbers! 
  ---------------------------------------------------------------------------*)
Definition mulB {n1 n2} (p1: BITS n1) (p2: BITS n2) :=
  fromNat (n:=n1+n2) (toNat p1 * toNat p2).
...
Definition rorB {n} (p: BITS n.+1) := let (p,b) := splitlsb p in joinmsb (b, p). 
...
Definition shrB {n} (p: BITS n.+1) := joinmsb0 (droplsb p).
...
Definition shlBaux {n} (p: BITS n)  := joinlsb (p, false).

(* Shift left: shift one bit to the left, put 0 in lsb, lose msb *)
Definition shlB {n} (p: BITS n.+1)  := dropmsb (shlBaux p).

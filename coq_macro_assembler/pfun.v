...
Section PFun.
  ...
  Lemma splitsAs_functional f g h i : splitsAs h f g -> splitsAs i f g -> h =1 i.
  Proof.
    rewrite /splitsAs. move => Hh Hi x.
    specialize (Hh x); specialize (Hi x).
    case eh:(h x) => [y |]; rewrite eh in Hh;
    case ei:(i x) => [z |]; rewrite ei in Hi;
    intuition congruence.
  Qed.

  Lemma splitsAs_functionalArg s s1 s2 s3 : splitsAs s s2 s1 -> splitsAs s s3 s1 -> s2 =1 s3. 
  Proof. 
    rewrite /splitsAs. move => H1 H2 x. 
    specialize (H1 x). specialize (H2 x). 
    case E1: (s x) => [y |]. rewrite E1 in H1, H2.
      destruct H1. destruct H as [Ha Hb]. 
      destruct H2; destruct H; congruence. 
      destruct H2; destruct H; destruct H0; congruence. 
    rewrite E1 in H1, H2. destruct H1; destruct H2; congruence. 
  Qed. 

  Lemma splitsAs_eq f g h i : splitsAs h f g -> h =1 i -> splitsAs i f g.
  Proof.
    rewrite /splitsAs. move => H heqi x.
    specialize (H x).
    case e:(h x) => [y |]; rewrite e in H; 
    rewrite heqi in e; rewrite e; intuition.
  Qed.

  Lemma splitsAs_eq_arg f g h i : splitsAs i f g -> f =1 h -> splitsAs i h g.
  Proof.
    rewrite /splitsAs. move => H feqh x.
    specialize (H x).
    case e:(i x) => [y |]; rewrite e in H; intuition congruence.
  Qed.
  ...
  * move => x. specialize (H1 x). specialize (H2 x). 
  ...
  Global Instance eqfun_equivalence {A B} : Equivalence (@eqfun A B).
  Proof.
    split.
    + intros a x; reflexivity.
    + intros a b H x; symmetry; apply H.
    + intros a b c H1 H2 x; transitivity (b x); [apply H1 | apply H2].
  Qed.

  Global Instance splitsAs_m :
    (Proper (@eqfun (option Y) X ==> @eqfun (option Y) X ==> @eqfun (option Y) X ==> iff)) splitsAs.
  Proof.
    move => a b aeqb c d ceqd e f eeqf; split. 
    + move => H.
      eapply splitsAs_eq; [|eassumption].
      eapply splitsAs_eq_arg; [|eassumption].
      apply splitsAs_commutative.
      eapply splitsAs_eq_arg; [|eassumption].
      apply splitsAs_commutative; assumption.
    + move => H.
      eapply splitsAs_eq; [|symmetry; eassumption].
      eapply splitsAs_eq_arg; [|symmetry; eassumption].
      apply splitsAs_commutative.
      eapply splitsAs_eq_arg; [|symmetry; eassumption].
      apply splitsAs_commutative; assumption.
  Qed.

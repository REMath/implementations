...
Lemma spec_frame S R:
  S |-- S @ R.
Proof.
  move => k P H. rewrite /spec_at /=.
  have HPR: extSP P (P ** R) by exists R.
...
Instance AtEx_forall A S {HS: forall a:A, AtEx (S a)} :
  AtEx (Forall a, S a).
Proof.
  move=> T f. setoid_rewrite spec_at_forall. apply: lforallR => a.
  setoid_rewrite <-at_ex. cancel1 => x. by apply lforallL with a.
Qed.
...
Module Export PullQuant.
  ...
  Hint Resolve PullQuant_forall : pullquant.

  Lemma PullQuant_propimpl p S:
    PullQuant (p ->> S) (fun H: p => S).
  Proof. red. reflexivity. Qed.
  Hint Resolve PullQuant_propimpl : pullquant.
...
Definition spec_reads S R := Forall s, (eq s |-- R) ->> S @ eq s.
...
Lemma spec_reads_alt S R:
  S <@ R -|- Forall s, (eq s |-- R ** ltrue) ->> S @ eq s.
Proof.
  ...
  - specintros => s Hs. rewrite <-lentails_eq in Hs.
    ...
    etransitivity; [apply (spec_frame (eq sframe))|]. autorewrite with push_at.
    rewrite stateSplitsAs_eq; [|eassumption]. done.
  - cancel1 => s. apply: lpropimplR => Hs. apply lpropimplL => //.
    rewrite ->Hs. by ssimpl.
Qed.

(* This definition can be more convenient in metatheory. *)
Lemma spec_reads_alt_meta S R : S <@ R -|- Forall s, R s ->> S @ eq s.
...
Proof.
  ...
  - specintros => s' Hs'. rewrite <-lentails_eq in Hs'. by subst s'.
Qed.

Lemma spec_reads_merge S R1 R2:
  S <@ R1 <@ R2 |-- S <@ (R1 ** R2).
Proof.
  repeat setoid_rewrite spec_reads_alt_meta.
  specintros => s [s1 [s2 [Hs [Hs1 Hs2]]]].
  apply lforallL with s2. apply lpropimplL; first done.
  assert ((Forall s', R1 s' ->> S @ eq s') |-- S @ eq s1) as Htmp.
...
Proof.
  ...
  rewrite (ILFun_exists_eq (eq s1 ** eq s2)).
...
(*
   Properties of spec_later
*)

Program Definition spec_later (S: spec) :=
  mkspec (fun k P => forall k', k' < k -> S k' P) _ _.
Next Obligation.
  move=> S k P P' HP H k' Hk. rewrite <-HP. auto.
Qed.
 
Notation "|> S" := (spec_later S) (at level 45, right associativity).

Lemma spec_later_weaken S:
  S |-- |> S.
Proof.
  move=> k P H. rewrite /spec_later /=. move=> k' Hk'.
  have Hge: k >= k' by omega. rewrite ->Hge in H. assumption.
Qed.

Instance spec_later_entails:
  Proper (lentails ==> lentails) spec_later.
Proof.
  move=> S S' HS [|k] P.
  - move=> _ /= k' Hk'. inversion Hk'.
  - move/(_ k P): HS => /= HS. move=> H' k' Hk'.
    have Hge: k >= k' by omega.
    rewrite <-Hge. apply HS. apply H'. auto.
Qed.

Instance spec_later_equiv:
  Proper (lequiv ==> lequiv) spec_later.
Proof.
  move=> S S' HS. split; cancel1; by rewrite HS.
Qed.

(* The Lob rule, which is the equivalent of induction in spec. *)
Lemma spec_lob S: (|> S |-- S) -> |-- S.
Proof.
  move=> H k P.
  elim: k => [|k].
  - move/(_ 0 P): H => /= H _. apply: H => k' Hk'. inversion Hk'.
  - move/(_ k.+1 P): H => /= H. move/(_ I) => HSn _. apply: H => k' Hk'.
    have Hge: k >= k' by omega. by rewrite <-Hge.
Qed.
...
Proof. split => k P /=; rewrite /spec_fun /=; eauto. Qed.

Lemma spec_later_and S S': |>(S //\\ S') -|- |>S //\\ |>S'.
Proof.
  split; rewrite /spec_later /= /spec_fun /= => k P H; split; by apply H.
Qed.

Lemma spec_later_impl S S': |>(S -->> S') -|- |>S -->> |>S'.
Proof.
  split.
  - apply: limplAdj. rewrite <-spec_later_and. cancel1.
    exact: landAdj.
  - rewrite /= /spec_fun /= => k P H.
    move=> k' Hk' k'' Hk'' P' HP HS.
    move/(_ k''.+1): H. apply; eauto with arith; [].
    move=> k''' Hk'''. have Hge: k'' >= k''' by omega. by rewrite <-Hge.
Qed.

Lemma spec_later_forall {T} F: |> (Forall x:T, F x) -|- Forall x:T, |> F x.
Proof. split; rewrite /spec_later /= /spec_fun /=; auto. Qed.

Lemma spec_later_exists {T} F: Exists x:T, |> F x |-- |> (Exists x:T, F x).
Proof. rewrite /spec_later /= /spec_fun /= => k P [x Hx]. eauto. Qed.

Lemma spec_later_exists_inhabited {T} F:
  inhabited T -> |> (Exists x:T, F x) |-- Exists x:T, |> F x.
Proof.
  move => [t] k. rewrite /spec_later /= /spec_fun /=. move=> P.
  case: k => [|k] => H.
  - exists t => k' Hk'. inversion Hk'.
  - have [|x Hx] := H k _; first by constructor. exists x => k' Hk'.
    have Hge: k >= k' by omega. by rewrite ->Hge in Hx.
Qed.

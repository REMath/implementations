...
namespace BEEV {  
  ...
  void BVSolver::VarsInTheTerm(const ASTNode& term, ASTNodeMultiSet& Vars) {
    ...
    while (nodes.size() > 0)
    {
      ASTNode node = nodes.top();
      nodes.pop();
      if (TermsAlreadySeenMap.find(node) != TermsAlreadySeenMap.end())
      {
        continue;
      }
      TermsAlreadySeenMap[node] = ASTTrue;
      ASTVec c;
      switch (node.GetKind())
      {
        case BVCONST:	continue;
        case SYMBOL: 	Vars.insert(node);
			continue;
        case READ:	if (node[0].GetKind()) 
                        {
			  nodes.push(node[1]);
                        }
                        else
			...
			continue;
        default:	c = node.GetChildren();
			for (ASTVec::reverse_iterator rit = c.rbegin(); rit != c.rend(); rit++)
			{
			  nodes.push(*rit);
			}
			continue;

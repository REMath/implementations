...
Section StateMT.
  ...
  Proof. 
  assert (H1:forall Z, (fun z:S*Z => let (st,x) := z in retn (st, x)) = fun z => retn z).
  ...
  (* id_l *) move => X c. apply functional_extensionality => s. 
  rewrite /=. rewrite H1/=. by rewrite H2 id_r. 
  (* id_r *) move => X Y Z c f g. apply functional_extensionality => s.
  rewrite /= assoc/=. apply f_equal. 
  apply functional_extensionality. by elim. 
  Qed.   

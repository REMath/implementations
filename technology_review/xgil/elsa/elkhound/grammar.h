...
public:	    // data
  ...
  Terminal *errorRecovery;      // error recovery production for the specified terminal.
                                // if specified then 'right' will be empty.
...
public:	    // data
  ...
  // the special terminal for error recovery. these will never show
  // up in the token stream, but will be pushed onto the stack if
  // a parse error is encountered which can be recovered from.
  LocString errorName;
  Terminal *errorTerm;

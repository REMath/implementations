...
:- use_module('inheritance', [basicGeneration/2 as clpGenerate, emptyState/2 as clpEmptyState]).
...
statement(skip, State, State) :- maybe(0.33932135425603616).
statement(E, IState, OState) :-
    decBound(IState, IStateB),
    maybe(0.435787358077743),
    expression(E, IStateB, OState).
statement(seq(S1, S2), IState, OState) :-
    decBound(IState, IStateB),
    maybe(0.07679872508662078),
...
template(seq(functionDecl(X,_, _), seq(simpleUpdate(access(var(X), str(prototype)), str(_), _), seq(new(var(X), _), skip)))).

templateSpecificity(NumToMake, NumMatched) :-
    emptyState(State, 9),
    templateCalc(State, NumToMake, 0, NumMatched).

timeCallFail(Timeout, Clause) :-
	catch(call_with_time_limit(Timeout, Clause), time_limit_exceeded, false).

templateCalc(_, 0, Accum, Accum) :- !.
templateCalc(State, NumToMake, NumMatchedAccum, NumMatched) :-
	!, 
	statistics(runtime, [Total, _]),
	(Total > 600000 -> throw(time_limit_exceeded); true),
	template(Statement),
	(timeCallFail(10, statement(Statement, State, _)) ->

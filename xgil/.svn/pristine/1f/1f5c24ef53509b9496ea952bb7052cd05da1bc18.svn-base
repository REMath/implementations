...
public:
  ...
  // number of parse errors which were encountered, including any errors
  // which were not recovered from.
  int errorCount;

  // after each error is encountered and recovered, number of tokens
  // to successfully lex before considering the parse fully recovered
  // and resuming error message printing. default is 5.
  int baseErrorTokenNumber;

  // number of tokens we still have to lex before the parse is fully
  // recovered from the most recent error. 0 if we are fully recovered.
  int errorTokenNumber;
...
private:    // funcs
  ...
  void recoverParse(StackNode *singleNode);
  void printParseErrorMessage(StateId lastToDie);
  void cleanupAfterParse(SemanticValue &treeTop);
  void nondeterministicParseToken();
  static void innerGlrParse(GLR &glr, LexerInterface &lexer, SemanticValue &treeTop);
  ...
  void rwlShiftTerminals(bool is_error = false);
...
public:     // funcs
  ...
  void glrParse(LexerInterface &lexer, SemanticValue &treeTop);
